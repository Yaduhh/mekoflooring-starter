        ...
        <div class="max-w-4xl mx-auto p-6 bg-white dark:bg-zinc-800 rounded-xl shadow-lg">
            ...
            <form action="{{ route('categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name_category" id="name_category" class="mt-1 block w-full bg-transparent border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-white rounded-md shadow-sm" value="{{ old('name_category', $category->name_category) }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="slug_category" id="slug_category" class="mt-1 block w-full bg-transparent border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-white rounded-md shadow-sm" value="{{ old('slug_category', $category->slug_category) }}" required readonly>
                </div>
                ...
                <div>
                    ...
                    @if ($category->image_category)
                        <div class="mt-2">
                            <img src="{{ Storage::url($category->image_category) }}" alt="{{ $category->name_category }}" class="w-32 h-32 rounded-lg">
                        </div>
                    @endif
                </div>
                ...
                <!-- Tombol Submit -->
                <div>
                    <button type="submit" class="w-full bg-blue-600 text-white p-2 rounded-md hover:bg-blue-700 focus:ring-4 focus:ring-blue-500 focus:outline-none">{{ __('Update Kategori') }}</button>
                </div>
            </form>
        </div>
    </div>
